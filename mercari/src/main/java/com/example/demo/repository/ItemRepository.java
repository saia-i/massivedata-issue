...
public class ItemRepository {
	...
	public List<Item> searchByNameAndBrand(String name, String brand, int startId) {
		String sql = "SELECT id,name,condition_id,category,brand,price,shipping,description FROM items WHERE name ilike :name AND brand ilike :brand ORDER BY id DESC limit 30 OFFSET :startId; ";
	 ...
	 */
	public int count() {
		String sql = "SELECT count(name) AS count FROM items;";
	...
	/**
	 * ブランド名から商品情報を完全一致検索し、その総件数を取得します.
	 * 
	 * @param brand ブランド名
	 * @return 総数
	 */
